...
# We need to use a DataFrame which is the pandas equivalent of a excel spreadsheet - it has rows and columns 
...
uniqueWords = set() 
wordDictionarySplit = dict()
# print(dataFrame)
# # head() prints out the first n rows of the csv file 
# print(dataFrame.head(200))

def cleanTransaction(text: str) -> str: 
...
dataFrame["cleaned"] = dataFrame["Company"].apply(cleanTransaction)

# Iterates through the cleaned column of the companies in the dataFrame, and adds each word into a set 
...
for string in dataFrame["cleaned"]: 
    words = string.split()
    for word in words: 
...
for i in range(0,len(uniqueWords)): 
   wordDictionarySplit[uniqueWords[i]] = i; 
...
termFrequencyMatrix = numpy.zeros((len(dataFrame["cleaned"]),len(wordDictionarySplit)))
   
# Function to find the number of times term t appears in transaction d 
# Index is the index of the transaction
def countTermInTransaction(t: str, index: int) -> int: 
  return (dataFrame["cleaned"][index]).count(t)

# Return number of words in trasnaction t 
# Index is the index of the transaction
def totalWordsInTransaction(index: int) -> int:
  return len((dataFrame["cleaned"][index]).split())
...
def termFrequency(): 
   cleanedTransactionsList = list(dataFrame["cleaned"])
   for i in range(0, len(cleanedTransactionsList)):
      ...
      for word in words: 
         if word in wordDictionarySplit: 
            matrixColumnIndex = wordDictionarySplit[word]
            termFrequencyMatrix[i, matrixColumnIndex] += 1

      termFrequencyMatrix[i] = termFrequencyMatrix[i] / totalWordLength

termFrequency()

# Create a vector for the inverseTermFrequency
inverseTermFrequencyVector = numpy.zeros(len(wordDictionarySplit))
...
def inverseDocumentFrequency(): 
   ...
   for i in range(0, len(cleanedTransactionsList)):
      ...
      for word in words: 
         if (word in wordDictionarySplit):
            inverseTermFrequencyVector[wordDictionarySplit[word]] += 1 
   
   return numpy.log(N / inverseTermFrequencyVector)
...
tfIDF = numpy.multiply(termFrequencyMatrix, inverseDocumentFrequency())
print(pandas.DataFrame(tfIDF))
print(numpy.sum(tfIDF[:,9]))
